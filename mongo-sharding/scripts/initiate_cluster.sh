...
until mongosh --host shard1 --port 27017 --eval "print(\"waited for connection\")"
do
    sleep 2
done
echo "Connection finished"
echo "Creating replica set"
mongosh --host shard1 --port 27017 <<EOF
  rs.initiate(
      {
        _id : "shard1",
        members: [
...
until mongosh --host shard2 --port 27017 --eval "print(\"waited for connection\")"
do
    sleep 2
done
echo "Connection finished"
echo "Creating replica set"
mongosh --host shard2 --port 27017 <<EOF
  rs.initiate(
      {
        _id : "shard2",
        members: [
